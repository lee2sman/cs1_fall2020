...
Build off the work we did in class to finish your Frogger-derived game. You can make your game more or less difficult depending on the speed. Consider the speed of the frog, and of the cars. It is okay to deviate from the classic Frogger gameplay and make your own version (in fact, it's encouraged). Use functions, classes and objects. Pay attention to functionality. Make the game a challenge, but make it playable. Use variables, not *magic* numbers (ie hardcoded numbers). Make a frog class and a car class. Use the arrow keys to control the frog. Consider how we checked whether the pong ball was on top of the rectangle. Use that knowledge to check whether the frog has hit a car. Add a score and lives.
